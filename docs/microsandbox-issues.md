...
- **Prevent OAuth refresh stampedes** (`544209b`). Process-local
  ...
  limits.
- **Prefer persisted OAuth over Claude auth environment overrides**
  (`pending`). Claude can ignore its persisted `.credentials.json` when
  auth override variables are present in the subprocess environment. Bullpen
  defensively removes those overrides from Claude subprocess envs after copying
  a usable VM-local OAuth file into the isolated `CLAUDE_CONFIG_DIR`.
...
### To distinguish stale OAuth from bad environment overrides

Check presence only; do not print token values. The diagnostic should report:

- whether Claude auth override environment variables are present
- whether `/home/bullpen/.claude/.credentials.json` exists
- whether the OAuth payload has access and refresh material
- the access-token TTL, if the file exposes one
...
Later the same day, after a rebuild, an older runtime home still had a
non-empty refresh token but a direct `claude --print` probe failed with HTTP
401. The failing Claude process then left Microsandbox-home with an empty
`refreshToken` and expired access token. That confirmed two distinct states:
